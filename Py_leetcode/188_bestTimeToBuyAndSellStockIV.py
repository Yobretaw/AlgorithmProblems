...
"""
    ...
    Formally , this problem is to compute the maximum value of (A[j_0] - A[i_0]) + (A[j_1] - A[i_1])
    + ... + (A[j_k-1] - A[i_k-1]) subject to i_0 < j_0 < i_1 < j_1 < .. < i_k-1 < j_k-1.
    A straightforward algorithm is to iterate over j from 1 to k and iterate through A, recording
    ...
    we can reduce to additional space complexity to O(n).

    We can improve the time complexity to O(kn), and the additional space complexity to O(k) as follows.
    Define B(i, j) to be the most money you can have if you must make j - 1 buy-sell transaction prior
    to i_th day and buy at i_th day. Define S(i, j) to be the maximum profit achievable with j buys
    and sells with the j_th sell taking place at i_th day. Then the following mutual recurrence holds:
...
def max_profit(prices, k):
    ...
    # f[i][j] = max profit gained by making at most j transactions from the first day to day i
    #f = [[0 for i in range(0, n)] for j in range(0, k + 1)]
    #for num in range(1, k + 1):
    #    # netval at day i = the max profit can be obtained by making (num - 1) transactions
    #    # in i - 1 days and buy(minus) stock(price) at day i.
    #    # To maximize netval by max(netval, mem[num - 1][day - 1] - prices[day])
    #    netval = -prices[0]
    #    for day in range(1, n):
    #        f[num][day] = max(f[num][day - 1], prices[day] + netval)
    #        netval = max(netval, f[num - 1][day - 1] - prices[day])
